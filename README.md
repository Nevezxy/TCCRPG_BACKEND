...
Checklist antes de publicar uma nova versão em produção:

1. Configurar todas as [variáveis de ambiente](#variáveis-de-ambiente) no
   serviço do Render (nunca commitar valores reais).
2. Confirmar `DEBUG` **não** definido como `True`.
3. Confirmar que o domínio de produção está em `ALLOWED_HOSTS`,
   `CSRF_TRUSTED_ORIGINS` e `CORS_ALLOWED_ORIGINS` (`app/settings.py`).
4. Rodar `python manage.py migrate` (feito automaticamente pelo `build.sh`).
5. Rodar `python manage.py collectstatic` (idem).

Arquivos estáticos são servidos via **WhiteNoise**
(`whitenoise.middleware.WhiteNoiseMiddleware`).

## Troubleshooting básico

- **`RuntimeError` na subida da aplicação mencionando `SECRET_KEY`**: a
  variável de ambiente `SECRET_KEY` não está definida. Configure-a no
  `.env` (local) ou nas variáveis de ambiente do serviço de deploy.
- **Erro de conexão com o banco ao rodar localmente sem Postgres**: o
  projeto assume `DATABASE_URL` com `ssl_require=True`. Para rodar contra
  um Postgres local sem SSL, ajuste esse parâmetro em `app/settings.py`
  (ou use uma instância de Postgres com SSL habilitado).
- **Upload de imagem falha silenciosamente**: confirme que as três
  variáveis `CLOUDINARY_*` estão configuradas e corretas.
- **`403 Forbidden` inesperado em um endpoint autenticado**: a maioria dos
  recursos "de mundo" de uma campanha usa `visivel_para_jogadores`/
  `editavel_para_jogadores` — um jogador (não-mestre) só lê recursos
  visíveis e só edita os que também são editáveis; exclusão é sempre
  exclusiva do mestre. Veja `Usuario/permissions.py::IsOwnerOrAdmin`.
- **`401 Unauthorized` logo após o login funcionar**: confirme que o
  header `Authorization: Bearer <access_token>` está sendo enviado, e que
  o token não expirou (ver prazos de expiração em
  `app/settings.py::SIMPLE_JWT`).

---

Para o relatório completo de auditoria de código, segurança e performance
realizado sobre este projeto, ver [`docs/AUDIT.md`](docs/AUDIT.md).
